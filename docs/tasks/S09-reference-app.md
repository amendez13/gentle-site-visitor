...
| Evidence rows are written as `{event_type, payload}` while older task text expected an `event` field. | `apps/example/README.md` and PR smoke output | Aligned in S10 docs and README examples with the S5 evidence sink schema; remaining app-author polish tracked in [#41](https://github.com/amendez13/gentle-site-visitor/issues/41). | yes |
| The S9 smoke contract expects `actions_total` and `cancellation_boundary`, but the runner naturally emits `requests_made`, `cooldowns`, and hydration counters. | `apps/example/visit.py` | S10 added `framework_counters_version: 1` and `framework.*` counter copies; app-specific normalized counters remain app-owned for now. | yes |
